...
basari=cross_val_score(estimator=classification,X=x_train,y=y_train,cv=4)

print(basari.mean())  #accuracy 1'e yakın iyi
print(basari.std())   #0'a yakın iyi
print(classification.score(x_test,y_test)) 

#en iyi algoritmayı bulmak

from sklearn.model_selection import GridSearchCV
p=[{"kernel":["rbf","linear","sigmoid"],"C":[1,2,3,4,5],
    "gamma":[1,0.5,0.1,0.01]},
   {"kernel":["poly"],"C":[1,2,3,4,5],
    "gamma":[1,0.5,0.1,0.01],"degree":[1,2,3,4]}]

gs=GridSearchCV(estimator=classification,param_grid=p,
